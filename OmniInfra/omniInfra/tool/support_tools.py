...
def run_python_repl(command: str) -> str:
    ...
    def execute_in_repl(command: str) -> str:
        ...
        try:
            ...
            # 资源代理异常必须穿透 REPL，交给 Worker 识别为资源等待超时或
            # 取消；普通工具异常仍保持历史上的字符串返回协议。
            if getattr(e, "propagate_to_caller", False):
                raise
...
def _apply_matplotlib_patches():
    ...
    try:
        ...
        if hasattr(plt, "_omniInfra_patched"):
            return
