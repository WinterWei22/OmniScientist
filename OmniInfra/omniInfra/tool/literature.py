...
def _fetch_public_url(
    ...
    *,
    ...
    request_headers = {"User-Agent": "OmniInfra/1.0 (+https://github.com/snap-stanford/omniInfra)"}
    if headers:
...
def query_arxiv(query: str, max_papers: int = 10) -> dict:
    ...
    try:
        ...
        return {
            "success": False,
            "error": "The optional 'arxiv' package is not installed. Rebuild or update the OmniInfra environment.",
        }
...
def query_biorxiv(
    ...
    def fetch_page(cursor: int) -> tuple[list[dict], dict]:
        ...
        response = requests.get(
            page_url,
            params=params,
            headers={"User-Agent": "OmniInfra/1.0 (+https://github.com/snap-stanford/omniInfra)"},
            timeout=timeout_seconds,
        )
        response.raise_for_status()
...
def _query_crossref_scholar_fallback(query: str, primary_error: str) -> dict | None:
    ...
    try:
        response = requests.get(
            ...
            headers={"User-Agent": "OmniInfra/1.0 (+https://github.com/snap-stanford/omniInfra; mailto:omniInfra@stanford.edu)"},
...
def query_pubmed(
    ...
    try:
        ...
        contact_email = email or os.getenv("OMNIINFRA_PUBMED_EMAIL")
        pubmed_kwargs = {"tool": "OmniInfra"}
        if contact_email:
