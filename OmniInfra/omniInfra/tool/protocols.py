...
try:
    ...
    from omniInfra.config import default_config  # type: ignore
except Exception:
...
ACCESS_TOKEN = os.getenv("PROTOCOLS_IO_ACCESS_TOKEN") or os.getenv("OMNIINFRA_PROTOCOLS_IO_ACCESS_TOKEN")
...
def search_protocols(
    ...
    if not ACCESS_TOKEN:
        raise ValueError(
            "Protocols.io access token is not configured. Set PROTOCOLS_IO_ACCESS_TOKEN or OMNIINFRA_PROTOCOLS_IO_ACCESS_TOKEN env var, or configure OmniInfraConfig.protocols_io_access_token."
        )
...
def _get_protocols_directory():
    ...
    return os.path.join(os.path.dirname(omniInfra.__file__), "tool", "protocols")


def list_local_protocols(source: str | None = None) -> dict[str, Any]:
    """
    List available protocol files in the local omniInfra/tool/protocols/ directory.
...
def read_local_protocol(filename: str, source: str | None = None) -> dict[str, Any]:
    """
    Read the contents of a local protocol file from omniInfra/tool/protocols/.
