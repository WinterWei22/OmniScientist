...
def _tools_pkg_root(repository_root: Path | None = None) -> Path:
    ...
    configured = os.environ.get("OMNIINFRA_TOOLS_PKG_ROOT", "").strip()
    if configured:
...
def _resolve_allocated_gpu(gpu_device: int | None, use_gpu: bool = True) -> int | None:
    ...
    if gpu_device is not None:
        return gpu_device
    allocated = os.environ.get("OMNIINFRA_ALLOCATED_GPU", "").strip()
...
def screen_compounds_with_drugclip(
    ...
    try:
        ...
        omniInfra_root = Path(__file__).resolve().parents[2]
        tool_home = Path(os.environ.get("DRUGCLIP_HOME", _tools_pkg_root(omniInfra_root) / "DrugCLIP")).expanduser().resolve()
        ...
        try:
            ...
            fallback_root = Path(tempfile.gettempdir()) / "omniInfra_drugclip_runs"
...
def rank_hits_with_graphdta(
    ...
    try:
        ...
        if not os.path.isfile(runner):
            return _hit_error(f"GraphDTA OmniInfra inference adapter is missing: {runner}", query_info)
...
def run_diffdock_with_smiles(pdb_path, smiles_string, local_output_dir, gpu_device=None, use_gpu=True, model_dir=None):
    try:
        ...
        published_dir = os.environ.get("OMNIINFRA_TASK_OUTPUT_DIR")
        if published_dir:
...
def docking_autodock_vina(
    ...
    if not prepare_receptor or not prepare_ligand:
        return {
            ...
            "error": (
                ...
                "Install meeko in the active OmniInfra Python environment or set "
                "MEEKO_PREPARE_RECEPTOR and MEEKO_PREPARE_LIGAND."
            ),
        }

    destination = Path(output_dir or os.environ.get("OMNIINFRA_TASK_OUTPUT_DIR") or "docking_results").expanduser().resolve()
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="omniInfra_vina_") as temp_dir:
...
def _process_ddinter_data_inline(data_lake_path, output_dir):
    ...
    data structures for use in OmniInfra drug-drug interaction analysis.
...
def query_drug_interactions(drug_names, interaction_types=None, severity_levels=None, data_lake_path=None):
    ...
    if data_lake_path is None:
        # Default path assuming standard OmniInfra structure
...
class OpenFDAClient:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "OmniInfra-Agent/1.0 (https://omniInfra.stanford.edu)"})
...
def predict_pka_with_tripka(
    ...
    runtime_root = os.path.abspath(
        os.path.expanduser(
            ...
            or "/tmp/omniInfra_tripka_runtime"
        )
    )
...
def _run_molecular_design_adapter(
    ...
    try:
        ...
        if not adapter_path.is_file():
            return _hit_error(f"OmniInfra adapter is missing: {adapter_path}", query_info)
...
def detect_protein_pockets_with_fpocket(
    ...
    from omniInfra.tool._a1_evidence_tools import detect_protein_pockets_with_fpocket_impl
...
def score_protein_pockets_with_dogsite(
    ...
    from omniInfra.tool._a1_evidence_tools import score_protein_pockets_with_dogsite_impl
...
def _run_molecule_generation_backend(
    ...
    try:
        ...
        if os.environ.get("OMNIINFRA_ALLOCATED_GPU", "").strip():
            child_environment["CUDA_VISIBLE_DEVICES"] = os.environ["OMNIINFRA_ALLOCATED_GPU"].strip()
        completed = subprocess.run(
            command,
            capture_output=True,
            text=True,
...
def generate_molecules_with_reinvent(config_path: str, output_dir: str, gpu_device: int | None = None, use_gpu: bool = True, timeout_seconds: int = 1800) -> dict:
    ...
    try:
        child_environment = os.environ.copy()
        if use_gpu and os.environ.get("OMNIINFRA_ALLOCATED_GPU", "").strip():
            child_environment["CUDA_VISIBLE_DEVICES"] = os.environ["OMNIINFRA_ALLOCATED_GPU"].strip()
        completed = subprocess.run(
            command,
            cwd=run_dir,
...
def _run_synthesis_adapter(
    ...
    if not isinstance(timeout_seconds, int) or timeout_seconds < 1:
        return _hit_error("timeout_seconds must be a positive integer.", query_info)
    if not adapter_path.is_file():
        return _hit_error(f"OmniInfra synthesis adapter is missing: {adapter_path}", query_info)
    if not runtime_python.is_file() or not os.access(runtime_python, os.X_OK):
        return _hit_error(
...
def plan_retrosynthesis_with_askcos(
    ...
    payload = {
        ...
        "description": "OmniInfra retrosynthesis request",
        "tags": "omniInfra",
        "expand_one_options": {
            "template_count": 100,
