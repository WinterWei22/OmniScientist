...
roots.  Public OmniInfra tools call :func:`prepare_request`, :func:`submit_request`
...
INTERNAL_TOOL_NAME = "omniInfra.tool._proteinmd_task._execute_proteinmd_inference"
...
class ProteinMDSettings:
    ...
    def from_env(cls) -> ProteinMDSettings:
        required_paths = {
            ...
            for name in (
                "OMNIINFRA_PROTEINMD_ROOT",
                "OMNIINFRA_PROTENIX_ROOT",
                "OMNIINFRA_PROTENIX_CHECKPOINT",
                "OMNIINFRA_PROTEINMD_CHECKPOINT_ROOT",
                "OMNIINFRA_PROTEINMD_CCD_CIF",
                "OMNIINFRA_PROTEINMD_CCD_RDKIT",
                "OMNIINFRA_PROTEINMD_ATLAS_INPUT_ROOT",
                "OMNIINFRA_PROTEINMD_MISATO_INPUT_ROOT",
                "OMNIINFRA_PROTEINMD_LEGACY_DATA_ROOT",
                "OMNIINFRA_PROTEINMD_STATE_ROOT",
                "OMNIINFRA_PROTEINMD_BWRAP",
            )
        }
        ...
        gpu_text = os.getenv("OMNIINFRA_PROTEINMD_GPU_DEVICES", "").strip()
        if not gpu_text:
            raise ValueError("OMNIINFRA_PROTEINMD_GPU_DEVICES is required")
        try:
            gpu_devices = tuple(sorted({int(item.strip()) for item in gpu_text.split(",") if item.strip()}))
        except ValueError as exc:
            raise ValueError("OMNIINFRA_PROTEINMD_GPU_DEVICES must be comma-separated integers") from exc
        ...
        sandbox_mode = os.getenv("OMNIINFRA_PROTEINMD_SANDBOX_MODE", "required").strip().lower()
        if sandbox_mode not in {"required", "disabled"}:
            raise ValueError("OMNIINFRA_PROTEINMD_SANDBOX_MODE must be 'required' or 'disabled'")
        return cls(
            proteinmd_root=Path(required_paths["OMNIINFRA_PROTEINMD_ROOT"]),
            protenix_root=Path(required_paths["OMNIINFRA_PROTENIX_ROOT"]),
            protenix_checkpoint=Path(required_paths["OMNIINFRA_PROTENIX_CHECKPOINT"]),
            checkpoint_root=Path(required_paths["OMNIINFRA_PROTEINMD_CHECKPOINT_ROOT"]),
            ccd_cif=Path(required_paths["OMNIINFRA_PROTEINMD_CCD_CIF"]),
            ccd_rdkit=Path(required_paths["OMNIINFRA_PROTEINMD_CCD_RDKIT"]),
            atlas_input_root=Path(required_paths["OMNIINFRA_PROTEINMD_ATLAS_INPUT_ROOT"]),
            misato_input_root=Path(required_paths["OMNIINFRA_PROTEINMD_MISATO_INPUT_ROOT"]),
            legacy_data_root=Path(required_paths["OMNIINFRA_PROTEINMD_LEGACY_DATA_ROOT"]),
            state_root=Path(required_paths["OMNIINFRA_PROTEINMD_STATE_ROOT"]),
            gpu_devices=gpu_devices,
            token_ttl_seconds=int(os.getenv("OMNIINFRA_PROTEINMD_TOKEN_TTL_SECONDS", "900")),
            termination_grace_seconds=int(os.getenv("OMNIINFRA_PROTEINMD_TERMINATION_GRACE_SECONDS", "60")),
            max_input_bytes=int(os.getenv("OMNIINFRA_PROTEINMD_MAX_INPUT_BYTES", str(20 * 1024**3))),
            max_output_bytes=int(os.getenv("OMNIINFRA_PROTEINMD_MAX_OUTPUT_BYTES", str(50 * 1024**3))),
            sandbox_mode=sandbox_mode,
            bwrap_path=Path(required_paths["OMNIINFRA_PROTEINMD_BWRAP"]),
        )
...
def _caller_id() -> str:
    return os.getenv("OMNIINFRA_TASK_ID", "local-direct-call")
...
def _validate_installation(settings: ProteinMDSettings, request: ProteinMDRequest) -> None:
    ...
    workspace_text = os.getenv("OMNIINFRA_TASK_DIR", "").strip() or os.getenv("OMNIINFRA_TASK_ROOT", "").strip()
    if not workspace_text:
        raise ValueError("OMNIINFRA_TASK_DIR or OMNIINFRA_TASK_ROOT is required for ProteinMD disk preflight")
...
def _task_service() -> Any:
    try:
        from tutorials.examples.expose_omniInfra_server.task_queue import TaskService
    except ImportError as exc:
        raise RuntimeError("OmniInfra task backend is unavailable in this deployment") from exc
...
def get_request(task_id: str) -> dict[str, Any]:
    try:
        ...
        if (
            ...
            and task_result.get("protocol") == "omniInfra.result.v1"
...
def _allocated_gpu() -> int:
    value = os.getenv("OMNIINFRA_ALLOCATED_GPU")
...
def _attempt_directories() -> tuple[Path, Path, Path, Path]:
    raw = os.getenv("OMNIINFRA_ATTEMPT_DIR")
    if not raw:
...
def execute_request(validation_token: str, gpu_device: int) -> dict[str, Any]:
    ...
    if token_record.get("child_task_id") != os.getenv("OMNIINFRA_TASK_ID"):
    ...
    cancel_files = (
        ...
        Path(os.getenv("OMNIINFRA_TASK_DIR", str(attempt))) / "CANCEL_REQUESTED",
    )
    if any(path.exists() for path in cancel_files):
