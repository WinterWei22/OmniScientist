...
def prepare_proteinmd_inference(
    dataset: str,
    system_id: str,
    ...
    """Validate a declarative ProteinMD request and issue a short-lived token.

    This read-only preflight does not start GPU inference. Runtime paths,
    checkpoints, commands and environment variables are administrator-managed
    and cannot be supplied through this interface.
    """
    from omniInfra.tool._proteinmd_runtime import prepare_request
...
def submit_proteinmd_inference(validation_token: str, gpu_policy: str = "auto") -> dict:
    """Submit a prevalidated ProteinMD request as an independent GPU tool task."""
    from omniInfra.tool._proteinmd_runtime import submit_request

    return submit_request(validation_token, gpu_policy)


def get_proteinmd_inference(task_id: str) -> dict:
    """Return status and validated result references for a ProteinMD task."""
    from omniInfra.tool._proteinmd_runtime import get_request

    return get_request(task_id)
