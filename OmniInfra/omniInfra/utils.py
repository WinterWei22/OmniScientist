...
def run_with_timeout(func, args=None, kwargs=None, timeout=600):
    ...
    # A1 的 Python REPL 在独立线程中运行；复制 contextvars 才能让运行时
    # GPU 代理看到 Worker 激活的任务资源上下文。
    execution_context = contextvars.copy_context()

    def thread_func(func, args, kwargs, result_queue):
        """Function to run in a separate thread."""
        try:
            result = execution_context.run(func, *args, **kwargs)
    ...
    try:
        ...
        if status == "success":
            return result
        # ResourceWaitTimeoutError/ResourceAcquisitionCancelledError 被运行时
        # GPU 代理标记为需要穿透；这样 Worker 才能重试或执行 shutdown 回队。
        if getattr(result, "propagate_to_caller", False):
            raise result
        return f"Error in execution: {result}"
    except queue.Empty:
...
def api_schema_to_langchain_tool(api_schema, mode="generated_tool", module_name=None):
    if mode == "generated_tool":
        module = importlib.import_module("omniInfra.tool.generated_tool." + api_schema["tool_name"] + ".api")
    ...
    # Create fields for both required and optional parameters. Previously the
    # optional portion of a OmniInfra schema was silently discarded here.
    annotations = {}
    fields = {}
    required_parameters = api_schema.get("required_parameters", [])
    optional_parameters = api_schema.get("optional_parameters", [])

    for param in required_parameters + optional_parameters:
        param_type = param["type"]
        if param_type in type_mapping:
            python_type = type_mapping[param_type]
        else:
            # For types not in the mapping, try a safer approach than direct eval
            try:
                python_type = eval(param_type)
            except (NameError, SyntaxError):
                # Default to Any for unknown types
                python_type = Any

        is_optional = param in optional_parameters
        default = param.get("default") if is_optional else ...
...
def read_module2api():
    ...
    module2api = {}
    for field in fields:
        module_name = f"omniInfra.tool.tool_description.{field}"
        module = importlib.import_module(module_name)
        module2api[f"omniInfra.tool.{field}"] = module.description
    return module2api
...
def check_and_download_s3_files(
    ...
    Args:
        s3_bucket_url: Base URL of the S3 bucket (e.g., "https://omniInfra-release.s3.amazonaws.com")
...
def parse_tool_calls_from_code(code: str, module2api: dict, custom_functions: dict = None) -> list[str]:
    ...
    Example:
        >>> code = "from omniInfra.tool import analyze_data\nimport pandas as pd"
...
def inject_custom_functions_to_repl(custom_functions: dict):
    ...
    if custom_functions:
        ...
        from omniInfra.tool.support_tools import _persistent_namespace
        ...
        builtins._omniInfra_custom_functions.update(custom_functions)
...
def format_detected_tools(detected_tool_modules: list) -> str:
    ...
    Example:
        >>> format_detected_tools([("analyze_data", "omniInfra.tool"), ("pandas", "pandas")])
        "omniInfra → analyze_data, pandas → pandas"
    """
    tool_descriptions = []
...
def remove_emojis_from_text(text: str) -> str:
    ...
    Note:
        The function targets specific emojis used in the OmniInfra system:
        - 🔧 for tools
        - 📊 for data
        - ⚙️ for software
        - 📋 for configuration
...
def convert_markdown_to_pdf(markdown_path: str, pdf_path: str) -> None:
    ...
    try:
        ...
        <head>
            <meta charset="utf-8">
            <title>OmniInfra Conversation History</title>
