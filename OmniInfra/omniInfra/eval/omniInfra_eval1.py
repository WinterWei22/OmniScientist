"""
OmniInfraEval1: Evaluation loader for OmniInfra tasks

This class provides a unified interface to evaluate user answers against ground truth
for all tasks in the OmniInfraEval1 benchmark.
"""
...
class OmniInfraEval1:
    """
    Evaluation loader for OmniInfraEval1 benchmark

    Usage:
        evaluator = OmniInfraEval1('omniInfra_eval1_dataset.parquet')
        score = evaluator.evaluate('gwas_causal_gene_opentargets', 0, 'BRCA1')
    """

    def __init__(self, dataset_path: str | None = None):
        """
        Initialize the OmniInfraEval1 evaluator

        Args:
            dataset_path: Path or URL to the merged dataset parquet file. When omitted,
                the public Hugging Face dataset is used.
        """

        if dataset_path is None:
            dataset_path = "hf://datasets/omniInfra/Eval1/omniInfra_eval1_dataset.parquet"
        ...
        print(f"Loaded OmniInfraEval1 dataset: {len(self.df)} instances across {self.df['task_name'].nunique()} tasks")
    ...
    def __repr__(self):
        return f"OmniInfraEval1(instances={len(self.df)}, tasks={self.df['task_name'].nunique()})"
...
def main():
    """Demo usage of OmniInfraEval1"""
    evaluator = OmniInfraEval1()
