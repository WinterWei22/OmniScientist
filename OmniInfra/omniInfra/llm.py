...
if TYPE_CHECKING:
    from omniInfra.config import OmniInfraConfig
...
def resolve_provider_model(model: str, source: str) -> str:
    ...
    ``Qwen3.8`` is intentionally a stable OmniInfra alias.  DashScope's
    ...
    variant with ``OMNIINFRA_QWEN38_MODEL`` without changing tool code.
    """
    if source == "Qwen" and model.strip().lower() in {"qwen3.8", "qwen-3.8"}:
        return os.getenv("OMNIINFRA_QWEN38_MODEL", "qwen3.8-max")
    return model
...
def get_llm(
    ...
    config: Optional["OmniInfraConfig"] = None,
    ...
    Args:
        ...
        config (OmniInfraConfig): Optional configuration object. If provided, unspecified parameters will use config values
    """
