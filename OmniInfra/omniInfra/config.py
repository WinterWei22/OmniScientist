"""
OmniInfra Configuration Management
...
@dataclass
class OmniInfraConfig:
    """Central configuration for OmniInfra agent.
    ...
    Usage:
        ...
        config = OmniInfraConfig()

        # Override specific settings
        config = OmniInfraConfig(llm="gpt-4", timeout_seconds=1200)
    ...
    def __post_init__(self):
        ...
        if os.getenv("OMNIINFRA_PATH") or os.getenv("OMNIINFRA_DATA_PATH"):
            self.path = os.getenv("OMNIINFRA_PATH") or os.getenv("OMNIINFRA_DATA_PATH")
        if os.getenv("OMNIINFRA_TIMEOUT_SECONDS"):
            self.timeout_seconds = int(os.getenv("OMNIINFRA_TIMEOUT_SECONDS"))
        if os.getenv("OMNIINFRA_LLM") or os.getenv("OMNIINFRA_LLM_MODEL"):
            self.llm = os.getenv("OMNIINFRA_LLM") or os.getenv("OMNIINFRA_LLM_MODEL")
        if os.getenv("OMNIINFRA_TOOL_LLM") or os.getenv("OMNIINFRA_TOOL_LLM_MODEL"):
            self.tool_llm = os.getenv("OMNIINFRA_TOOL_LLM") or os.getenv("OMNIINFRA_TOOL_LLM_MODEL")
        if os.getenv("OMNIINFRA_TOOL_SOURCE"):
            self.tool_source = os.getenv("OMNIINFRA_TOOL_SOURCE")
        if os.getenv("OMNIINFRA_USE_TOOL_RETRIEVER"):
            self.use_tool_retriever = os.getenv("OMNIINFRA_USE_TOOL_RETRIEVER").lower() == "true"
        if os.getenv("OMNIINFRA_NETWORK_POLICY"):
            self.network_policy = os.getenv("OMNIINFRA_NETWORK_POLICY", "controlled").strip().lower()
        if os.getenv("OMNIINFRA_COMMERCIAL_MODE"):
            self.commercial_mode = os.getenv("OMNIINFRA_COMMERCIAL_MODE").lower() == "true"
        if os.getenv("OMNIINFRA_TEMPERATURE"):
            self.temperature = float(os.getenv("OMNIINFRA_TEMPERATURE"))
        if os.getenv("OMNIINFRA_MAX_OUTPUT_TOKENS"):
            self.max_output_tokens = int(os.getenv("OMNIINFRA_MAX_OUTPUT_TOKENS"))
        if os.getenv("OMNIINFRA_CONTEXT_WINDOW_TOKENS"):
            self.context_window_tokens = int(os.getenv("OMNIINFRA_CONTEXT_WINDOW_TOKENS"))
        if os.getenv("OMNIINFRA_TOKEN_SAFETY_MARGIN"):
            self.token_safety_margin = int(os.getenv("OMNIINFRA_TOKEN_SAFETY_MARGIN"))
        if os.getenv("OMNIINFRA_CUSTOM_BASE_URL"):
            self.base_url = os.getenv("OMNIINFRA_CUSTOM_BASE_URL")
        if os.getenv("OMNIINFRA_CUSTOM_API_KEY"):
            self.api_key = os.getenv("OMNIINFRA_CUSTOM_API_KEY")
        if os.getenv("OMNIINFRA_SOURCE"):
            self.source = os.getenv("OMNIINFRA_SOURCE")

        # Protocols.io access token (prefer specific env vars)
        env_token = os.getenv("PROTOCOLS_IO_ACCESS_TOKEN") or os.getenv("OMNIINFRA_PROTOCOLS_IO_ACCESS_TOKEN")
        if env_token:
...
default_config = OmniInfraConfig()
