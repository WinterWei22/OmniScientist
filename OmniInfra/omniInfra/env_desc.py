...
data_lake_dict = {
    ...
    "DisGeNET.parquet": (
        "Gene-disease associations from multiple sources. Key columns: Disorder (str) and Genes (str). "
        "Genes stores a string representation of a Python list, not a native list; parse it with "
        "ast.literal_eval before filtering, membership checks, or set operations."
    ),
    ...
    "omim.parquet": (
        "Genetic disorders and associated genes from OMIM. Key columns: Phenotypes (str), Approved Gene "
        "Symbol (str), and Gene/Locus And Other Related Symbols (str). Match diseases in Phenotypes, use "
        "Approved Gene Symbol for canonical genes, and treat the related-symbol field as comma-separated aliases."
    ),
    ...
    "txgnn_name_mapping.pkl": "Trusted, versioned TxGNN mapping artifact containing an id2name_drug dictionary; deploy with provenance, license, and SHA-256 metadata.",
    "txgnn_prediction.pkl": "Trusted, versioned precomputed TxGNN disease-to-drug raw ranking scores; values are not calibrated treatment probabilities.",
}
...
library_content_dict = {
    ...
    "geodiff": "[Isolated External Tool] MIT-licensed geometric diffusion model for generating 3-D conformers of a supplied molecular graph. OmniInfra uses a pinned pretrain-branch checkout, isolated Python environment, caller-supplied checkpoint, and scripts/molecular_design_adapters/run_tool.py.",
    "targetdiff": "[Isolated External Tool] MIT-licensed target-aware diffusion model for generating ligands from a prepared protein-pocket PDB. Requires a pinned checkout, isolated CUDA environment, official sampling YAML, and validated checkpoint.",
    "pocket2mol": "[Isolated External Tool] MIT-licensed model for sampling molecules in a caller-defined protein-pocket box. Requires a pinned checkout, isolated CUDA environment, official sampling YAML, and validated checkpoint.",
    "autogrow4": "[Isolated External Tool] Apache-2.0 evolutionary ligand optimization workflow using reaction-based mutation/crossover and docking. Requires its isolated runtime plus upstream molecule-preparation and docking executables.",
    "syntalinker": "[Isolated External Tool] MIT-licensed conditional Transformer for joining two single-attachment molecular fragments under a shortest-linker-bond-distance constraint. OmniInfra uses a pinned checkout and isolated legacy-compatible CPU environment; inference requires a separately supplied, reviewed checkpoint because upstream publishes no pretrained weights.",
    "deeppurpose": "[Python Package] A deep learning library for drug-target interaction and ADMET prediction. OmniInfra prefers extracted official checkpoints under DEEPPURPOSE_PRETRAINED_ROOT. ADMET inference requires 16 endpoint checkpoints per selected encoding and disables upstream downloads by default.",
    "descriptastorus": "[Python Package] DeepPurpose's molecular descriptor backend; install the official bp-kelley descriptastorus package.",
    "pandas-flavor": "[Python Package] DeepPurpose runtime dependency used by its data processing utilities.",
    "pytorch-geometric": "[Python Package] Graph neural network framework required by the official GraphDTA architectures; install a version compatible with the installed PyTorch.",
    "drugclip": "[External Model] Offline official DrugCLIP retrieval through tools_pkg/DrugCLIP/run_drugclip.py. Set DRUGCLIP_PYTHON to the isolated omniInfra_drugclip interpreter and use CUDA. The runner converts candidate SMILES to a molecule LMDB with RDKit and accepts either a cropped pocket PDB/ENT or an official pocket LMDB; it loads checkpoint_best.pt from tools_pkg/DrugCLIP by default. Deployment and optional overrides are documented in tools_pkg/DrugCLIP/README_OMNIINFRA.md.",
    "graphdta": "[External Model] Official GraphDTA checkout and a user-supplied trained checkpoint. Set GRAPH_DTA_REPO; install PyTorch Geometric and dependencies. The upstream repository supplies training code but no general pretrained checkpoint.",
    "pharmit": "[External Service] Pharmit FastCGI service (default https://pharmit.csb.pitt.edu/fcgi-bin/pharmitserv.fcgi) or a self-hosted pharmitserver deployment. Set PHARMIT_URL for another endpoint. Pharmit is BSD/GPL dual licensed; OpenBabel-dependent deployments are GPL.",
    "zinc15": "[External API] Public ZINC15 substance API at https://zinc15.docking.org/substances.json. Requests use explicit timeouts; the service may require browser human verification and can block automated requests.",
    ...
    "ADFR": "[External CLI] Official Scripps ADFR Suite used by run_autosite. Deploy executable prepare_receptor and autosite commands on PATH or set ADFR_PREPARE_RECEPTOR and AUTOSITE_BIN; Meeko is not an AutoSite replacement.",
