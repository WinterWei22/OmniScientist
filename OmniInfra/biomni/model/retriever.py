...
from biomni.model.embedding_cache import QwenEmbeddingCache
...
class QwenSemanticResourceRanker:
    ...
    DEFAULT_INSTRUCTION = (
        "Given a multi-step biomedical research task, decide whether this single Biomni resource directly supports "
    ...
    def __init__(
        ...
        *,
        ...
        self.embedding_backend = (embedding_backend or os.getenv("BIOMNI_EMBEDDING_BACKEND", "api")).lower()
        if self.embedding_backend not in {"api", "local"}:
            raise ValueError("BIOMNI_EMBEDDING_BACKEND must be 'api' or 'local'")
        self.embedding_model_name = embedding_model or (
            os.getenv("BIOMNI_EMBEDDING_MODEL", "qwen3.7-text-embedding")
            if self.embedding_backend == "api"
            else os.getenv("BIOMNI_EMBEDDING_MODEL_PATH", "Qwen/Qwen3-Embedding-4B")
        )
        self.reranker_backend = (reranker_backend or os.getenv("BIOMNI_RERANKER_BACKEND", "api")).lower()
        if self.reranker_backend != "api":
            raise ValueError("Local Qwen reranking is no longer supported; set BIOMNI_RERANKER_BACKEND=api")
        requested_reranker_model = reranker_model or os.getenv(
            "BIOMNI_RERANKER_MODEL", os.getenv("BIOMNI_RERANKER_API_MODEL", "qwen3-rerank")
        )
        ...
        self.reranker_api_style = (
            os.getenv("BIOMNI_RERANKER_API_STYLE") or model_defaults.get("style") or "dashscope-native"
        ).lower()
        if self.reranker_api_style not in {"qwen-compatible", "dashscope-native"}:
            raise ValueError("BIOMNI_RERANKER_API_STYLE must be 'qwen-compatible' or 'dashscope-native'")
        self.reranker_base_url = (
            reranker_base_url
            or os.getenv("BIOMNI_RERANKER_BASE_URL")
        ...
        self.reranker_timeout_seconds = float(
            reranker_timeout_seconds or os.getenv("BIOMNI_RERANKER_TIMEOUT_SECONDS", "60")
        )
        self.device = device or os.getenv("BIOMNI_RETRIEVAL_DEVICE", "auto")
        self.embedding_batch_size = max(
            1,
            int(embedding_batch_size or os.getenv("BIOMNI_EMBEDDING_BATCH_SIZE", "16")),
        )
        self.reranker_batch_size = max(
            1,
            int(reranker_batch_size or os.getenv("BIOMNI_RERANKER_BATCH_SIZE", "16")),
        )
        self.max_length = max(128, int(max_length or os.getenv("BIOMNI_RETRIEVAL_MAX_LENGTH", "1024")))
    ...
    def _model_load_error(kind: str, model_name: str) -> RuntimeError:
        env_name = "BIOMNI_EMBEDDING_MODEL_PATH" if kind == "embedding" else "BIOMNI_RERANKER_MODEL"
        return RuntimeError(
