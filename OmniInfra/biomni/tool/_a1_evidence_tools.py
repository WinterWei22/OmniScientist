...
_PHAROS_QUERY = """
query BiomniPharosTarget($symbol: String!, $diseaseTop: Int!, $ligandTop: Int!) {
  target(q: {sym: $symbol}) {
    tcrdid
    sym
    name
...
query BiomniResolveTarget($queryString: String!) {
...
query BiomniTargetGenetics($ensemblId: String!, $page: Pagination!) {
...
def detect_protein_pockets_with_fpocket_impl(pdb_file_path: str, output_dir: str, top_n: int = 10) -> dict:
    ...
    container_name = "biomni_input.pdb"
    result_dir = output_path / "biomni_input_out"
    ...
    if error:
        return _failure(tool, query_info, error, source)
    info_file = result_dir / "biomni_input_info.txt"
