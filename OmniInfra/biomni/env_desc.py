...
library_content_dict = {
    ...
    "geodiff": "[Isolated External Tool] MIT-licensed geometric diffusion model for generating 3-D conformers of a supplied molecular graph. Biomni uses a pinned pretrain-branch checkout, isolated Python environment, caller-supplied checkpoint, and scripts/molecular_design_adapters/run_tool.py.",
    ...
    "syntalinker": "[Isolated External Tool] MIT-licensed conditional Transformer for joining two single-attachment molecular fragments under a shortest-linker-bond-distance constraint. Biomni uses a pinned checkout and isolated legacy-compatible CPU environment; inference requires a separately supplied, reviewed checkpoint because upstream publishes no pretrained weights.",
    "deeppurpose": "[Python Package] A deep learning library for drug-target interaction and ADMET prediction. Biomni prefers extracted official checkpoints under DEEPPURPOSE_PRETRAINED_ROOT. ADMET inference requires 16 endpoint checkpoints per selected encoding and disables upstream downloads by default.",
    ...
    "drugclip": "[External Model] Offline official DrugCLIP retrieval through tools_pkg/DrugCLIP/run_drugclip.py. Set DRUGCLIP_PYTHON to the isolated biomni_drugclip interpreter and use CUDA. The runner converts candidate SMILES to a molecule LMDB with RDKit and accepts either a cropped pocket PDB/ENT or an official pocket LMDB; it loads checkpoint_best.pt from tools_pkg/DrugCLIP by default. Deployment and optional overrides are documented in tools_pkg/DrugCLIP/README_BIOMNI.md.",
